package com.hc.mall.seckill.service.impl;
...
import com.hc.mall.seckill.feign.CouponFeignService;
import com.hc.mall.seckill.feign.ProductFeignService;
import com.hc.mall.seckill.interceptor.LoginInterceptor;
import com.hc.mall.seckill.service.SecKillService;
import com.hc.mall.seckill.to.SeckillSkuRedisTo;
import com.hc.mall.seckill.vo.SeckillSessionWithSkusVo;
import com.hc.mall.seckill.vo.SkuInfoVo;
...
public class SeckillServiceImpl implements SecKillService {
    ...
    private void saveSecKillSession(List<SeckillSessionWithSkusVo> sessions) {
        sessions.stream().forEach(session -> {
            String key = SESSION_CACHE_PREFIX + session.getStartTime() + "_" + session.getEndTime();
            //当前活动信息未保存
        ...
        if (!StringUtils.isEmpty(json)) {
            ...
            if (currentTime >= redisTo.getStartTime() && currentTime <= redisTo.getEndTime()) {
                ...
                if (redisKey.equals(killId) && redisTo.getRandomCode().equals(key)) {
                    ...
                    Boolean occupy = redisTemplate.opsForValue().setIfAbsent(memberResponseVo.getId() + "-" + redisTo.getSkuId(), num.toString(), ttl, TimeUnit.MICROSECONDS);
                    ...
                    if (occupy) {
                        ...
                        if (num <= redisTo.getSeckillLimit()) {
                            ...
                            boolean acquire = semaphore.tryAcquire(num, 100, TimeUnit.MICROSECONDS);
                            if (acquire) {
                                //5. 发送消息创建订单
                                //5.1 创建订单号
